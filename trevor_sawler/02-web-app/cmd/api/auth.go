...
func (app *application) getTokenFromHeaderAndVerify(w http.ResponseWriter, r *http.Request) (string, *Claims, error) {
	...
	_, err := jwt.ParseWithClaims(token, claims, func(token *jwt.Token) (interface{}, error) {
		// validate the signing algorithm (ensure it is signed with the algorithm we use)
	...
	// check if error message starts with "token is expired by"
	if err != nil {
...
func (app *application) generateTokenPair(user *data.User) (TokenPairs, error) {
	...
	// create the refresh token (users might not use) - don't need all the claims as it is in jwt access token
